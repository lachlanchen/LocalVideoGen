...
Choosing **Use references** in the Single Clip UI applies the separate **Long reference · 24 GiB safe** preset: 14 seconds, 25-step `quality_int8_offload`, reference matching, and a 1248×704 or 704×1248 canvas. The raw `POST /api/renders` compatibility default remains five seconds when an R2V caller omits `duration`; requests with a visual video also default to INT8/offload and 1248×704, while image/audio-only R2V keeps the earlier BF16 and 1344×768 defaults. Every visual video needs server-trusted normalized dimensions at every duration and is fitted to H3-native 32-pixel axes within 1024 px and 589,824 pixels (576×1024 for 9:16). Match-mode admission uses `aligned_frames × (output_pixels + sum(video_reference_pixels)) + max(0, matched_still_count − 1) × output_pixels`: each still after the calibrated first costs one output canvas once, not once per frame. The API rejects a base video-frame load above 510,000,000 at any duration; from 243 aligned frames (about 9.5 requested seconds), visual-video R2V also requires `quality_int8_offload` and `match`. The validated 14-second portrait case is 704×1248 + one 576×1024 reference + one matched still, 345 frames, or 506,603,520 proxy pixels. Additional stills or visual/continuity videos may require a shorter clip or smaller canvas; use audio-only guidance when pictures are unnecessary. The 510M/24 GiB claim applies to this measured match-mode route—explicit shorter BF16/max mixes remain available but are outside that guarantee.
...
- Pre-clean every visual video reference before upload and run the fail-closed [subtitle-free reference guard](docs/reference-subtitle-guard.md) on the exact 2–15-second segment. A prompt that says “no subtitles” cannot reliably remove caption pixels already present in the reference; use audio-only input when only voice or music continuity is needed.
...
- Highest-fidelity preset for a short visual-video reference or image-only R2V: pruned BF16 Ref2VA/FL2VA DiT, the selected aligned or Heretic NVFP4 Qwen3-VL conditioner, FP16 video VAE, FP32 audio VAE, and 25 full-model steps. In Series, image-only means **Continuity handoff: Off**; otherwise successor shots gain a visual continuity tail. Long visual-video references default to the measured INT8/offload safety envelope above.
- Shared-workstation stage placement: GPU 0 runs the DiT, denoiser, Qwen/reference conditioning, and final video/audio decode by default, leaving GPU 1 available for LocalLLM or another protected workload. For BF16 stage-placement profiles, this keeps the same model, BF16 precision, sampler, resolution, and 25-step quality while using GPU-0/CPU offload. On an exclusive workstation, deliberately set `H3_AUX_DEVICE=gpu:1` to move Qwen plus reference-conditioning VAE work to GPU 1; final decode still remains on GPU 0. No PCIe peer-to-peer path is assumed.
