...
长视频参考请使用 **Long reference · 24 GiB safe**：`quality_int8_offload`、`match`，以及竖屏 704×1248 或横屏 1248×704。提交前防护会检查尺寸和参考负载，超过安全上限时会在任务送入 GPU 前拒绝。
...
无需离开 H3 Studio，即可在 **Single Clip** 与 **Series** 之间切换。系列模式提供 **LALACHAN Series**、质量优先的 **World Travel** 预设，以及适合任意演员与视觉风格的中性 **My Movie** 预设。

![H3 Studio 浅色主题 World Travel 导演台](../docs/images/h3-studio-world-travel-light.png)

- 演员、世界观、声音和动作参考只需上传一次，然后可编排 2–12 张分镜卡；每张卡都能独立编辑提示词、时长和种子。
- **World Travel** 将七张标准角色/道具图锁定为 P1–P7，为每个分镜分配专属目的地场景图 P8，并将 P9 保留给上一个已验收分镜的精确最后一帧。以往剧集只能提供身份或声音参考；不得影响新国家、剧情、走位、色彩或构图。
...
- 其他本地项目和 Codex 会话可使用仅依赖 Python stdlib 的无第三方依赖 Series 客户端。它会在受限大小内上传参考素材、验证服务器能力，并在生成前以原子方式写入含持久 ID 的回执；暂停审核或轮询中断后仍可继续，还会在安装下载文件前核验产物大小与 SHA-256。

这一工作流借鉴了小云雀分镜体验的清晰度，但所有生成与项目状态都通过本机回环保留在这台工作站上；不会调用小云雀或任何付费云端生成服务。

有关连续性与恢复行为，请参阅[系列工作流指南](../docs/series-workflow.md)；有关 stdlib CLI/客户端及完整 HTTP 契约，请参阅[跨项目 Series API 指南](../docs/local-series-api.md)；有关可信的原生 H3 基线、实验性连续性项目、可选插帧与质量门禁，请参阅[流畅长视频方案评述](../docs/smooth-long-video-options.md)。

## 能力概览

- BF16 最高保真适用于短视频视觉参考或仅图像 R2V；长视频参考默认采用经过实测、适配 24 GiB 的安全 INT8/offload 路线。
- 在共享工作站上，默认由 GPU 0 运行全部 H3 阶段，GPU 1 留给 LocalLLM；只有显式设置 `H3_AUX_DEVICE=gpu:1` 才会把 Qwen 与参考 VAE 移到 GPU 1。
...
flowchart LR
    ...
    C --> G0[GPU 0: 默认运行全部 H3 阶段]
    G1[GPU 1: 默认留给 LocalLLM]
    C -. 可选 H3_AUX_DEVICE .-> G1
