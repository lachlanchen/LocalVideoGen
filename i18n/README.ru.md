...
В H3 Studio можно переключаться между **Single Clip** и **Series**, не покидая рабочее пространство. Режим серии содержит направляемый шаблон **LALACHAN Series** с семью именованными слотами персонажей и реквизита, а также нейтральный **My Movie** для любого состава и визуального стиля.

![Режиссёрская панель видеосерии в светлой теме H3 Studio](../docs/images/h3-studio-series-light.png)
...
- Единый шлюз допуска обеспечивает строго последовательный рендер H3. После полной проверки видео и звука каждого не последнего кадра его точный финальный кадр и последние три секунды становятся референсами непрерывности для следующего.
...
- Сохраняется каждая попытка. Итоговый фильм собирается без потерь копированием потоков только после проверки числа кадров, 24 fps, стереозвука, полного декодирования и SHA-256; рядом сохраняется манифест проверки.
